...
use polaris::api::{
    AccountResponse, CatalogResponse, CatalogSymbol, CatalogVenue, CliAuthPollResponse,
...
async fn remote_catalog_can_be_listed_without_filters() {
    ...
    assert_eq!(catalog.venues.len(), 1);
    assert_eq!(catalog.venues[0].id, fixture.venue);
    assert_eq!(catalog.venues[0].symbols.len(), 1);
    assert_eq!(catalog.venues[0].symbols[0].id, fixture.symbol);
}

#[tokio::test]
async fn missing_catalog_symbol_returns_dataset_unavailable() {
    let mut fixture = SnapshotFixture::basic();
    fixture.symbol_available = false;
    let server = TestServer::spawn(fixture.clone()).await;
    let tempdir = TempDir::new().expect("tempdir");
    let config = config_for_test(server.base_url(), tempdir.path());
    let client =
...
impl TestServer {
    async fn spawn(fixture: SnapshotFixture) -> Self {
        ...
        let state = TestServerState {
            ...
            symbol_available: fixture.symbol_available,
        };
        let app = Router::new()
...
async fn handle_catalog(
    ...
    let include_venue = query
        .venue
        .as_deref()
        .is_none_or(|value| value == state.venue);
    let include_symbol = query
        .symbol
        .as_deref()
        .is_none_or(|value| value == state.symbol);
    let symbols = if state.symbol_available && include_venue && include_symbol {
        vec![CatalogSymbol {
            id: state.symbol.clone(),
    ...
    Json(CatalogResponse {
        venues: vec![CatalogVenue {
            id: query.venue.unwrap_or(state.venue.clone()),
            symbols,
        }],
