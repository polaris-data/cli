...
function PolarisBrowser({ client, config, layout, seed, initialMarkets }: PolarisBrowserProps) {
  const { exit } = useApp();
  ...
  const dateGroups = useMemo(() => (plan ? groupByDate(plan.snapshots) : []), [plan]);
  ...
  const loadMarketDates = useCallback(
    async (market: BrowserDataset): Promise<LoadedMarketDates> => {
  ...
  const syncCachedPlan = useCallback(
    (nextPlan: SyncPlan) => {
  ...
  const enterMarket = useCallback(async () => {
    ...
    try {
      ...
      setFocus('dates');
      setStatus(
        `${loaded.plan.snapshots.length} snapshots across ${loaded.dateGroups.length} dates.`,
      );
    } catch (error) {
  ...
  const downloadDate = useCallback(
  ...
  const dateFocusWidths = allocateColumnWidths(
    columns,
    [0.18, 0.26, 0.5, 0.06],
    [12, 18, 22, 2],
    columnGap,
  ...
  const marketViewportEnd = Math.min(
    marketViewportStart + marketListHeight,
  ...
  const previewDateViewportEnd = Math.min(
    previewDateViewportStart + availableHeight,
    previewDateGroups.length,
  );
  const marketListHeader =
    !showSearchInput && marketViewportStart > 0
        ...
        <Box flexDirection="row">
          ...
          >
            <Box flexDirection="column" marginLeft={2}>
              ...
              <Text> </Text>
              <Text dimColor wrap="wrap">
                ↑↓ navigate · → enter market
              </Text>
              <Text dimColor wrap="wrap">
                ← back · type to filter
              </Text>
              <Text dimColor wrap="wrap">
                Enter download · Esc quit
              </Text>
              <Text> </Text>
              <Text> </Text>
          ...
          <Box flexDirection="column" flexGrow={1} marginRight={columnGap}>
            <Text dimColor={showSearchInput && !search} wrap="truncate-end">
              {marketListHeader}
            </Text>
            ...
            {!selectedMarket ? (
              ...
              previewDateGroups.length > 0 ? (
                <>
                  ...
                  {previewDateGroups
                    .slice(previewDateViewportStart, previewDateViewportEnd)
                    .map((dg) => {
                      ...
                      return (
                        <Text key={dg.date} dimColor wrap="truncate-end">
                          {'  '}
        ...
        <Box flexDirection="row">
          ...
          <Box flexDirection="column" flexGrow={1} marginRight={columnGap}>
            ...
            {filteredMarkets.slice(marketViewportStart, marketViewportEnd).map((market, i) => {
              ...
              const isSelected = index === selectedMarketIndex;
              return (
                <Text key={market.dataset} dimColor bold={isSelected} wrap="truncate-end">
                  {isSelected ? '▶ ' : '  '}
                  {market.dataset}
                </Text>
              );
          ...
          >
            {loadingDates ? (
              ...
              <>
                {dateViewportStart > 0 && <Text dimColor>↑ {dateViewportStart} more</Text>}
                {dateGroups.slice(dateViewportStart, dateViewportEnd).map((dg, i) => {
                  const index = dateViewportStart + i;
                  const isSelected = index === selectedDateIndex;
                  const downloadedNow =
                    downloading && dg.date === activeDownloadDate
...
async function loadDatasets(client: PolarisClient, seed: BrowserSeed): Promise<BrowserDataset[]> {
...
function allocateColumnWidths(
  ...
  let widths = ratios.map((ratio, index) =>
    Math.max(minimums[index] ?? 1, Math.floor(usableWidth * ratio)),
  );
